...
bot = Bot(
    token='6766163800:AAEiePkz6fEDRXdyAEF6s5lRsev4bo6220M',
...
async def config_update():
    ...
    while True:
        ...
        config = {
            'balances': gram_ton_balances,
            'wallets': gram_ton_wallets['wallets'],
            'config': gram_ton_config
        }
...
async def select_pair(query: types.CallbackQuery):
    ...
    keyboard = types.InlineKeyboardMarkup(row_width=1).add(
        types.InlineKeyboardButton(text=f'GRAM/TON', callback_data=f'exchange:GRAM_TON'),
...
async def pair_selected(data: types.CallbackQuery):
    ...
    await bot.edit_message_text(
        ...
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton(text='💸 Transfer TON', url=f'ton://transfer/{wallet_to_send_to}'),
...
async def pair_selected(data: types.CallbackQuery):
    ...
    await bot.edit_message_text(
        ...
        message_id=data.message.message_id,
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton(text='💸 Transfer GRAM', url=f'https://t.me/gram20bot/app'),
            types.InlineKeyboardButton(text='⬅️ Back', callback_data=f'exchange:{pair_from}_{pair_to}')
        )
    )
